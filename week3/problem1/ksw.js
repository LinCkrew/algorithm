...
input.forEach((item) => {
  if (!gather.some((element) => item.includes(element)))
  ...
  item.split('').some((element) => {
    ...
    if (
      newTwiceTemp === element.repeat(2) &&
      !twiceGather.includes(newTwiceTemp)
    ) {
      obj[item].result = `<${item}> is not acceptable.\n`;
      return true;
    } else {
      ...
      if (tripleTemp.length === 3) {
        ...
        const isTripleGather = splitTripleTemp.every((item) =>
          gather.includes(item),
        );
        const isTripleCons = splitTripleTemp.every(
          (item) => !gather.includes(item),
        );
